...
\NormalTok{  results <-}\StringTok{ }\KeywordTok{foreach}\NormalTok{(}\DataTypeTok{i=}\NormalTok{inputs,}\DataTypeTok{.export=}\KeywordTok{c}\NormalTok{(}\StringTok{"n_samples"}\NormalTok{, }\StringTok{"n_treatment"}\NormalTok{,}
            \StringTok{"generate_net_benefit"}\NormalTok{, }\StringTok{"generate_input_parameters"}\NormalTok{),}\DataTypeTok{.packages=}\StringTok{'MASS'}\NormalTok{) }\OperatorTok{%dorng%}\StringTok{ }
\StringTok{    }\NormalTok{\{}
...
Now use the mlmc() function to estimate the EVPPI of \(y\). This is a
function we use directly for the EVPPI results without the additional
test results. The input is similar to mlmc.test() function.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate the EVPPI for y}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{tst_y <-}\StringTok{ }\KeywordTok{mlmc}\NormalTok{(}\DataTypeTok{Lmin=}\DecValTok{2}\NormalTok{, }\DataTypeTok{Lmax=}\DecValTok{10}\NormalTok{, }\DataTypeTok{N0=}\DecValTok{1024}\NormalTok{, }\DataTypeTok{eps=}\DecValTok{7}\NormalTok{, }\DataTypeTok{mlmc_l =}\NormalTok{ EVPPI_y_l_p, }\DataTypeTok{gamma=}\DecValTok{1}\NormalTok{)}

\CommentTok{# EVPPI is lowest eps estimate }
\CommentTok{# Remember MLMC estimates DIFF from EVPI so need to subtract this from total EVPPI}
\CommentTok{# The EVPI in this case using 10^7 samples is 121}
\DecValTok{121} \OperatorTok{-}\StringTok{ }\NormalTok{tst_y}\OperatorTok{$}\NormalTok{P}
\end{Highlighting}
\end{Shaded}
